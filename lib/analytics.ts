...
class AnalyticsTracker {
  ...
  trackFunnelStep(step: string, metadata?: Record<string, any>) {
    ...
    this.trackEvent("funnel_step", "conversion", step, undefined, undefined, metadata)
  }
  ...
  private storeFunnelData() {
    localStorage.setItem("conversion_funnel_" + this.sessionId, JSON.stringify(this.conversionFunnel))
  }
  ...
  private calculateConversionRate(events: ConversionEvent[], sessions: UserSession[]): number {
    const conversions = events.filter((e) => e.action === "lead_generated").length
    return sessions.length > 0 ? (conversions / sessions.length) * 100 : 0
  }

  private getTopSources(sessions: UserSession[]) {
    const sources = sessions.reduce(
      (acc, session) => {
  ...
  private getFunnelDropoff() {
    ...
    const stepCounts = funnelSteps.reduce(
      (acc, step) => {
        acc[step] = 0
        return acc
      },
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith("conversion_funnel_")) {
        ...
        const stepsReached = new Set(funnel.map((f: ConversionFunnel) => f.step))
      ...
      dropoffRate:
        step === "assessment_started"
          ...
          : stepCounts[funnelSteps[funnelSteps.indexOf(step) - 1]] > 0
            ? ((stepCounts[funnelSteps[funnelSteps.indexOf(step) - 1]] - stepCounts[step]) /
  ...
  private getLeadQualityMetrics(events: ConversionEvent[]) {
    ...
    const qualityMetrics = {
      highActivity: leadEvents.filter((e) => e.customParameters?.activityLevel === "High").length,
      mediumActivity: leadEvents.filter((e) => e.customParameters?.activityLevel === "Moderate").length,
      lowActivity: leadEvents.filter((e) => e.customParameters?.activityLevel === "Low").length,
      averageConfidence: 0,
    }

    const confidenceScores = leadEvents
      .map((e) => Number.parseFloat(e.customParameters?.confidence?.replace("%", "") || "0"))
...
export const trackAssessmentProgress = (questionNumber: number, totalQuestions: number) => {
...
export const trackAssessmentComplete = (pestType: string, activityLevel: string, confidence: string) => {
  analytics.trackFunnelStep("assessment_completed", { pestType, activityLevel, confidence })
  analytics.trackEvent("assessment_complete", "conversion", "assessment_finished", pestType, undefined, {
    pestType,
    activityLevel,
    confidence,
  })
...
export const trackResultsViewed = (pestType: string, activityLevel: string) => {
  ...
  analytics.trackEvent("results_viewed", "engagement", "results_displayed", pestType)
}

export const trackContactFormOpened = (contactType: "call" | "sms") => {
  analytics.trackFunnelStep("contact_form_opened", { contactType })
  analytics.trackEvent("contact_form_opened", "conversion", "form_opened", contactType)
}
...
export const trackLeadGenerated = (
  ...
  analytics.trackFunnelStep("lead_generated", { contactType, pestType, activityLevel, confidence })
  analytics.trackEvent("lead_generated", "conversion", "lead_submitted", contactType, 1, {
    pestType,
    activityLevel,
    confidence,
    contactType,
...
export const trackChatbotInteraction = (message: string, response: string) => {
  analytics.trackEvent("chatbot_interaction", "engagement", "message_sent", undefined, undefined, {
    userMessage: message,
